...
export default function SettingsPage() {
  ...
  const inputClass = 'w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder:text-gray-400 text-sm focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-50 transition-all'
        ...
        <div className="flex-1 max-w-2xl">
          {activeSection === 'profile' && (
            <div className="glass rounded-2xl p-6 space-y-6">
              ...
              <div className="flex items-center gap-5">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white font-black text-2xl shadow-md shadow-amber-500/20">
                  {session?.user?.name?.[0]?.toUpperCase() || 'M'}
                </div>
                <div>
                  <button className="text-sm bg-white border border-gray-200 text-gray-700 px-4 py-2 rounded-xl hover:bg-gray-50 transition-all">
                    Change Avatar
                  </button>
          ...
          {activeSection === 'integrations' && (
            <div className="glass rounded-2xl p-6 space-y-4">
              <h2 className="font-bold text-gray-900 text-lg">Integrations</h2>
              <p className="text-gray-500 text-sm">Connect your trading platforms and community tools.</p>
              {[
                ...
                <div key={integration.name} className="flex items-center justify-between p-4 bg-gray-50 border border-gray-100 rounded-xl hover:border-gray-200 transition-all">
                  ...
                  <button className="text-xs bg-amber-50 border border-amber-200 text-amber-700 px-4 py-2 rounded-xl hover:bg-amber-100 transition-all">
                    Connect
                  </button>
                </div>
              ))}
          ...
          {(activeSection !== 'profile' && activeSection !== 'integrations') && (
            <div className="glass rounded-2xl p-6">
              <h2 className="font-bold text-gray-900 text-lg capitalize mb-4">
