...
import { Sparkles, RefreshCw, TrendingUp, TrendingDown, BarChart2 } from 'lucide-react'
...
function renderMarkdown(text: string) {
  // Simple markdown renderer for ## headers and line breaks
  return text.split('\n').map((line, i) => {
    if (line.startsWith('## ')) {
      return (
    ...
    if (line.startsWith('# ')) {
      return (
        <h1 key={i} className="text-xl font-black text-white mt-6 mb-3 first:mt-0">
          {line.replace('# ', '')}
        </h1>
      )
    }
    ...
    <div className="p-8 max-w-2xl">
      ...
      {!data && !loading && (
        <div className="glass rounded-2xl p-10 flex flex-col items-center text-center">
          ...
          <h2 className="text-lg font-bold text-white mb-2">Ready for your review?</h2>
          ...
          <button
            onClick={fetchReview}
            className="flex items-center gap-2 bg-gradient-to-r from-amber-500 to-orange-500 text-black font-bold px-8 py-3.5 rounded-xl hover:shadow-lg hover:shadow-amber-500/30 transition-all"
          >
      ...
      {loading && (
        <div className="glass rounded-2xl p-10 flex flex-col items-center text-center">
          ...
          <p className="text-gray-400 text-sm">Analyzing your week...</p>
          <p className="text-gray-600 text-xs mt-1">Reading your emotions, notes, and patterns</p>
        </div>
      )}
        ...
        <div className="space-y-5">
          ...
          {data.hasData && data.stats && (
            <div className="grid grid-cols-4 gap-3">
              <div className="glass rounded-xl p-4 text-center">
                ...
                <div className="text-xl font-black text-white">{data.stats.totalTrades}</div>
              </div>
              <div className="glass rounded-xl p-4 text-center">
                <div className="text-xs text-gray-500 uppercase tracking-wider mb-1">Win Rate</div>
                <div className="text-xl font-black text-amber-400">{data.stats.winRate}%</div>
              </div>
              <div className="glass rounded-xl p-4 text-center">
                <div className="text-xs text-gray-500 uppercase tracking-wider mb-1">W / L</div>
                <div className="text-xl font-black text-white">
                  <span className="text-emerald-400">{data.stats.wins}</span>
                  <span className="text-gray-600 mx-1">/</span>
                  <span className="text-red-400">{data.stats.losses}</span>
                </div>
              </div>
              <div className="glass rounded-xl p-4 text-center">
                <div className="text-xs text-gray-500 uppercase tracking-wider mb-1">P&L</div>
                <div className={cn('text-xl font-black', isPositive ? 'text-emerald-400' : 'text-red-400')}>
          ...
          <div className="glass rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-white text-sm">Claude&apos;s Analysis</span>
              </div>
              <button
                onClick={fetchReview}
          ...
          {!data.hasData && (
            <div className="glass rounded-2xl p-8 text-center">
              ...
              <p className="text-gray-400 text-sm">{data.review}</p>
              <p className="text-gray-600 text-xs mt-2">Log some trades and come back for your review.</p>
            </div>
          )}
        </div>
      )}
