...
export default function MarketOverviewPage() {
  ...
  return (
    <div className="p-8 max-w-6xl">
      {/* Header */}
      <div className="mb-10">
        <p className="text-gray-500 text-sm mb-1">{getTodayLabel()}</p>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Left column: Today's Briefing + Pre-Session */}
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="font-bold text-white">Today&apos;s Briefing</h2>
                <p className="text-xs text-gray-500">Live news feed — coming soon</p>
              </div>
            </div>
            <div className="space-y-3">
              {[
                ...
                <div key={item.headline} className="flex items-start gap-3 py-3 border-b border-white/5 last:border-0">
                  <span className="text-xs bg-white/5 border border-white/10 rounded-full px-2 py-0.5 text-gray-400 whitespace-nowrap mt-0.5">{item.tag}</span>
          ...
          <div className="glass rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="font-bold text-white">Pre-Session Analysis</h2>
                <p className="text-xs text-gray-500">AI brief — coming soon</p>
              </div>
            </div>
            <div className="rounded-xl border border-dashed border-white/10 bg-white/2 p-6 text-center">
              ...
              <p className="text-gray-600 text-xs mt-1">Analyzes your recent trades, key levels, and today&apos;s economic events.</p>
              <Link
                href="/ai-assistant"
        ...
        {/* Right column: Economic Calendar + Quick Links */}
        <div className="space-y-6">
          ...
          <div className="glass rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="font-bold text-white text-sm">Economic Calendar</h2>
                <p className="text-xs text-gray-500">Today&apos;s data releases</p>
              </div>
            </div>
            <div className="space-y-2">
              {economicEvents.map((ev) => (
                <div key={ev.event} className="rounded-xl bg-white/3 border border-white/5 p-3">
                  ...
                  <p className="text-sm font-medium text-white leading-tight">{ev.event}</p>
                  <div className="flex gap-4 mt-1.5 text-xs text-gray-500">
                    <span>Forecast: <span className="text-gray-300">{ev.forecast}</span></span>
                    <span>Prior: <span className="text-gray-300">{ev.prior}</span></span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="glass rounded-2xl p-5">
            ...
            <div className="space-y-2">
              {[
                { href: '/trades/new', icon: PlusCircle, label: 'Log Trade', color: 'text-amber-400', bg: 'bg-amber-400/10' },
                { href: '/calendar', icon: Calendar, label: 'View Calendar', color: 'text-blue-400', bg: 'bg-blue-400/10' },
                { href: '/ai-assistant', icon: Bot, label: 'Ask AI Assistant', color: 'text-purple-400', bg: 'bg-purple-400/10' },
              ].map((action) => (
                <Link
                  key={action.href}
                  href={action.href}
