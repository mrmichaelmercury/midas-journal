...
export default function PayoutsPage() {
  ...
  const inputClass = 'w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder:text-gray-400 text-sm focus:outline-none focus:border-amber-400 transition-all'
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-black text-gray-900">Payout Tracker</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Payouts', value: formatCurrency(total), icon: '💰', color: 'text-emerald-600' },
          { label: 'This Month', value: formatCurrency(thisMonth), icon: '📅', color: 'text-amber-600' },
          { label: 'Largest Single', value: formatCurrency(largest), icon: '🏆', color: 'text-purple-600' },
          { label: 'Total Count', value: `${payouts.length} payouts`, icon: '✅', color: 'text-blue-600' },
        ].map((s) => (
      ...
      <div className="glass-gold rounded-2xl p-6 mb-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div>
            <div className="text-sm text-amber-600 font-medium mb-1">🏆 Lifetime Earnings from Prop Firms</div>
            <div className="text-5xl font-black text-gray-900">{formatCurrency(total)}</div>
            <div className="text-gray-500 text-sm mt-2">Keep going. The Midas Touch is real. 🔥</div>
          </div>
          <div className="hidden md:block">
            <Wallet className="w-20 h-20 text-amber-300" />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {payouts.map((payout, i) => (
          <div key={payout.id} className="glass rounded-2xl p-5 flex items-center gap-5 hover:border-gray-300 transition-all">
            <div className={cn(
              'w-12 h-12 rounded-xl bg-gradient-to-br flex items-center justify-center text-white font-black text-lg flex-shrink-0',
              firmColors[payout.firm] || 'from-gray-400 to-gray-500'
            )}>
              {payout.firm[0]}
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-bold text-gray-900">{payout.firm}</div>
            ...
            <div className="text-right flex-shrink-0">
              <div className="text-2xl font-black text-emerald-600">+{formatCurrency(payout.amount)}</div>
              {i === 0 && <div className="text-xs text-amber-600 mt-0.5">Latest 🎉</div>}
            </div>
          </div>
        ))}
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass rounded-2xl p-7 w-full max-w-md border-amber-200 shadow-xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-black text-gray-900 text-xl">Log New Payout</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Prop Firm</label>
                <select value={form.firm} onChange={(e) => setForm({...form, firm: e.target.value})} className={inputClass}>
              ...
              <div>
                ...
                <input type="number" step="0.01" min="0" value={form.amount} onChange={(e) => setForm({...form, amount: e.target.value})}
                  placeholder="1,000.00" required className={inputClass} />
              </div>
              <div>
              ...
              <button type="submit" disabled={saving}
                className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold py-3.5 rounded-xl hover:shadow-lg hover:shadow-amber-500/25 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
